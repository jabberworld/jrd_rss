...
* Транспорт переписан на Python 3. Вместо pyxmpp используется slixmpp, вместо
  python-mysqldb - pymysql. Требуется Python 3.9+.
* Реализована поддержка XEP-0050 (Ad-Hoc Commands):
  * Для каждой ленты доступны команды: "Feed info", "My subscription",
    "Edit feed info", "Edit subscription", "Feed privacy" - просмотр и изменение
    параметров ленты и настроек подписки (описание, теги, интервал обновления,
    тип проверки уникальности, фильтры, приватность и т.п.).
  * На самом транспорте доступны команды: "Register new feed",
    "Show my feeds", "Show my private feeds", "Show all feeds",
    "Show Categories" - выбор функции происходит сразу, без промежуточного меню.
* Исправлено отображение лент в браузере сервисов: раньше при просмотре свойств
  ленты ее имя подменялось на "Jabber RSS Transport", теперь корректно
  показывается имя ленты и ее описание.
* Схема базы создается и обновляется автоматически при старте: не нужно вручную
  импортировать jrdrss.scheme.sql, а для существующих установок миграции из
  предыдущих версий применяются на лету.

## v 1.15.2

* Фикс для setposfilter / setnegfilter - на некоторых (!) хостах были вылеты из-за кириллицы в фильтрах.

## v 1.15.1

* Добавлена обработка блока yandex:full-text для тех лент, где тело ленты находится в этом блоке.
* Исправлена ошибка для лент с пустым телом и включенной проверкой уникальности на базе ссылки + заголовка + тела.

## v 1.15

* Фикс: дополнение в версии 1.14.8 сломало работу с не-ASCII JID'ами.
* Добавлена функция mute/unmute: теперь можно отключить получение новостей от ленты без бана контакта, но при этом сохраняется возможность получения новости по запросу (отправкой цифры контакту).
* В командах установки фильтров (``setposfilter`` и ``setnegfilter``), а также для ``setshort`` - т.е., тех, которые относятся к пользовательским настройкам, а не к администрированию ленты в целом, убрана необходимость указывать название ленты или ставить "``:``". Также упрощена setshort - убрана возможность вызова команды без аргументов для того, чтобы избежать случайного удаления настройки ленты.

Схема базы обновлена, см. каталог alts/

## v 1.14.8

* Фикс: добавлен ресурс не только для статусных станз, а и для сообщений
...
* Реализована поддержка XEP-0039 - вывод статистики. Реализация не вполне корректная, так как согласно документации она должна идти в 2 этапа, но на опробованных конференц-ботах она именно в таком виде и второй этап они пропускают. На обычных jabber-клиентах проверить не было возможности - ни в Psi, ни в Gajim функции не наблюдается.

## v 1.14.4

* Исправлена ошибка с добавлением +1 в число подписчиков при регистрации ленты через браузер сервисов - остался лишний код со старых времен. Запрос на изменение числа подписчиков при добавлении/удалении лент заменен на универсальный, считающий реальное число JID'ов в списке подписчиков.

## v 1.14.3

* Исправления в обработке тела новости - добавлены/убраны переносы строк при цитировании. Фикс с устранением лишних переносов, добавившихся после выноса обработки тела вне цикла со списком JID'ов. Убрал немного специфичных для Python2 моментов в коде (вроде ничего не сломал).
...
* Результаты в выборке из базы при запросе последних сообщений сгруппированы по ссылке для исключения дубликатов в случае, если контент новости менялся.

## v 1.14

* Добавлена возможность указания для ленты типа проверки уникальности новости - только по ссылке, по ссылке + заголовку или по ссылке + заголовку + контенту. Опция доступна как при регистрации ленты, так и в дальнейшем через команды от владельца ленты или админа. Дефолтное значение - сравнение по ссылке (раньше было по ссылке + заголовку, что иногда приводило к повторам сообщений с небольшими изменениями). Однако есть ленты (например, на некоторых форумах), где ни ссылка, ни заголовок не меняются, но обновляется содержимое - в таком случае поможет самый последний режим.
...
* Разрешено добавление администратором (через диалог "+ feedname ...") лент для серверов с некорректным полем Content Type (например, text/html вместо text/xml). Такие ленты должны корректно парситься, но на всякий случай стоит быть внимательным. В логах для таких лент пишется сообщение про Bozo flag: NonXMLContentType. Также исправлена ошибка с разбором сообщения при таком типе добавления: раньше, если описание ленты и единственный тег были одним словом, то описание не сохранялось.
...
* Добавлена команда для возможности поиска только в рамках определенного тега - ``searchintag`` или ``?!#`` - например, теперь можно делать запросы вида ``?!# hard ssd`` или ``?!# soft ssd`` с разными результатами.

## v 1.13

* Убрана зависимость от hashlib - в коде больше не используется md5 для хранения хэша для полученных новостей, так как в базе сейчас сохраняется оригинальная информация (заголовок и ссылка), по которой строился хэш, и можно использовать напрямую ее, не делая лишних вычислений и не храня лишнюю информацию. Схема базы обновлена, убрана колонка md5 в таблице sent. Для тех лент, где новости приходили до добавления архива сообщений и где в базе не хранится ссылка и заголовок на новость - такие новости могут прийти повторно.

## v 1.12

* Так как на сейчас в транспорте есть поиск и выборка последних новостей, то колонка datetime в таблице sent перестала быть простым маркером актуальности контента, как это было поначалу. В итоге логика сортировки и выборки данных ломалась. В этом релизе это исправлено: добавлена еще одна колонка, где хранится время занесения новости в таблицу sent. Также убрана давно уже неиспользуемая колонка received. Обновите схему базы - см. каталог alts.

## v 1.11.5

* Запрос на текущую длину сообщения теперь возвращает название для специальных режимов вместо числа (Unlimited, Title only и так далее).

## v 1.11.4

* Добавлен режим ``setshort NAME 3`` - отобразить только один абзац из тела новости - по аналогии с ``setshort NAME 2`` для одного предложения на случай, когда такого варианта будет мало.
